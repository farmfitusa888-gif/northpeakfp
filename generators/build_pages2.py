...
cards = "".join(f"""<a class="entry art" href="articles/{a['slug']}.html" data-cat="{html.escape(a['cat'])}"
...
hub_body = f"""
{crumb([("Home","index.html"),("Articles",None)])}
<div class="wrap pagehead">
...
W("articles.html", shell(
    ...
    canon=f"{SITE}/articles", body=hub_body, active="Articles",
...
W("404.html", shell(title=f"Page Not Found | {FIRM}",
  ...
  <a href="articles.html" class="btn ghost lg" style="margin-left:10px">Browse Articles</a>
  </div></section>"""))
...
redir += [f"/{p}{' ' * max(1, 25 - len(p))}/{p}.html{' ' * max(1, 13 - len(p))}200"
          for p in STATIC_PAGES]
redir += ["/articles                 /articles.html         200",
          "/articles/:slug           /articles/:slug.html   200",
          "/service-areas            /service-areas.html    200",
